...
int blinkCounter = 0;
const int blinkInterval = 200; // How many loops before blink
...
void handleRoot() {
  ...
  html += "<title>Pwnagotchi WiFi Spammer</title>";
  html += "<style>body{font-family:Arial;text-align:center;margin-top:50px;}";
  html += "button{padding:10px 20px;font-size:16px;margin:10px;}</style></head>";
  html += "<body><h1>ESP32 Pwnagotchi WiFi Beacon Spammer</h1>";
  html += "<p>Status: <strong>" + String(beaconSpamActive ? "ACTIVE" : "INACTIVE") + "</strong></p>";
...
void handlePwnagotchiEyes() {
  ...
  if (isBlinking) {
    drawBlinkingEyes();
    isBlinking = false; // Single blink
  } else {
    drawPwnEyes(offsetX, offsetY);
  }
...
void drawPwnEyes(int offsetX, int offsetY) {
  // Eye outlines
...
void drawBlinkingEyes() {
  // Just draw closed eyes (horizontal lines)
  display.drawLine(17, 32, 47, 32, SH110X_WHITE);  // Left eye closed
  display.drawLine(81, 32, 111, 32, SH110X_WHITE); // Right eye closed
}
